...
def solveCavity(K, sig_K, m, sig_m, mu, sig, gamma):
    ...
    print('convergence step size for cavity:',err)
...
dist_par = [S, M, gamma, sigma, mu, mu_K, mu_m, sigma_K, sigma_m]
...
np.savez(outfile, dist_par, cav_par, dataSet)
...
dVar = np.var(dataSet,axis =2)

parVar = np.mean(dVar,axis = 1)

print('variance in each parameter due to initial conditions:',parVar)
