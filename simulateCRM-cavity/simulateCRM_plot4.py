...
def solveCavity(K, sig_K, m, sig_m, mu, sig, gamma):
    ...
    return [phi_N, phi_R, avg_N, avg_R, q_N, q_R, nu, chi, err] #last element is the convergence error (or step size)


numVary = 15


rhoSet = np.linspace(.5,10, numVary);
mu =1;
sigma = mu*sqrt(1/(rhoSet**2) - 1.)
